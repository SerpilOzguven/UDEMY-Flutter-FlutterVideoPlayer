...
class VideoPlayerPageExample extends StatefulWidget {
  const VideoPlayerPageExample({super.key});
...
class _VideoPlayerPageExampleState extends State<VideoPlayerPageExample> {
  ...
  void initState() {
    super.initState();
    imagePicker = ImagePicker();
    //videoPlayerController = VideoPlayerController.networkUrl('datasource')..initilaze(){
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: controller == null ? Container() :
      buildVideo(),
    );
  }
  ...
  IconButton goButton() {
    return IconButton(onPressed: (){
                        controller!.seekTo(Duration(seconds: controller!.value.position.inSeconds+2));
                      },
  ...
  IconButton playButton() {
    return IconButton(onPressed: (){
                      ...
                      }, icon: controller!.value.isPlaying ? const Icon(Icons.play_circle_fill): const Icon(Icons.pause_circle_filled)
                      );
  }
  ...
  String time() {
    var duration = Duration(milliseconds: controller!.value.position.inMilliseconds);
    return [duration.inMinutes, duration.inSeconds].map((sag) => sag.remainder(60).toString().padLeft(2,'0')).join(':');
  }
  String time1() {
    var duration = Duration(milliseconds: controller!.value.duration.inMilliseconds);
    return [duration.inMinutes, duration.inSeconds].map((sag) => sag.remainder(60).toString().padLeft(2,'0')).join(':');
